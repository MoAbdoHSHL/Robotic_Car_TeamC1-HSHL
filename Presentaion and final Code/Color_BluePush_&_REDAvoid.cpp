// ------------------------------------------------------
// Line Following Robot with Color Obstacle Scenarios
//
// Copy Right for Group (C1) HSHL
//
// Members:
// - Mohamed Abdo
...
//
// Project Overview:
// This Arduino code controls a line-following robot that can detect colored obstacles
// and react differently to each color:
//
// </> RED Obstacle </>:
//    The robot backs up, checks if the left side is clear, and if so, bypasses the obstacle by going around it,
//    then returns to the lane and continues.
// </> BLUE Obstacle (FLIPPED with GREEN) </>:
//    The robot moves forward to push the obstacle, then reverses back on the lane. After that, the ultrasonic
//    sensor is disabled for 5 seconds to avoid detecting the same obstacle again.
// </> GREEN or OTHER Colors (FLIPPED with BLUE) </>:
//    The robot backs up, scans left and right, and if the right side is clear, parks to the right and stops.
//
// The robot follows a black line using infrared sensors, uses an ultrasonic sensor to detect obstacles ahead,
// and a color sensor to recognize the obstacle color and trigger the correct scenario.
//
...
void handleDetectedColor(String color) {
  ...
  if (color == "RED") {
    ...
    Serial.println("BLUE scenario: Forward 40cm, then reverse 40cm on lane, ignore ultrasonic for 5s.");
    driveForward(20, 150);
    Stop(); delay(300);
    ...
    ultrasonicEnabled = true;
    Serial.println("Ultrasonic re-enabled, BLUE scenario complete: Lane reacquired.");
    Stop();
  } else if (color == "GREEN" || color == "OTHER") {
    Serial.println("GREEN/OTHER COLOR scenario: Back 10cm, scan right/left, park if right is clear.");
