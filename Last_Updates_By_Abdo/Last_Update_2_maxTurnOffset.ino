...
void loop() {
  ...
  // Lane following logic with P-like control
  ...
  if (leftIR == 0 && rightIR == 0) {
    ...
    // Curve left by slowing left motor
    moveForward(baseTurnSpeed, baseTurnSpeed + maxTurnOffset);
  } else if (leftIR == 0 && rightIR == 1) {
    // Curve right by slowing right motor
    moveForward(baseTurnSpeed + maxTurnOffset, baseTurnSpeed);
  } else {
    Stop(); delay(100);
  }
}
...
void handleDetectedColor(String color) {
  if (color == "RED") {
    ...
    Stop(); Serial.println("BLUE maneuver complete → Parked");
    while (true);
  } else {
